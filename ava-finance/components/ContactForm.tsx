...
const field =
  "w-full rounded-xl border border-line bg-canvas px-4 py-3 text-ink placeholder:text-ink-faint focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20 transition-all";
...
export default function ContactForm() {
  ...
  if (sent) {
    return (
      <div className="flex h-full min-h-64 flex-col items-start justify-center gap-3">
        <span className="grid h-12 w-12 place-items-center rounded-none bg-accent/10 text-2xl text-accent">
          ✓
        </span>
        <span className="font-display text-2xl text-ink">Thank you.</span>
  ...
  return (
    ...
    >
      ...
      <button
        type="submit"
        className="group inline-flex items-center gap-2 self-start rounded-none bg-accent px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-[#1842c2]"
      >
        Send message
